...
与 middle_layer.models.AgentAnalysisInput配套：每个JD只分析一次，通过target_ids关联申请岗位。只表达分析与面试建议，不修改招聘状态。
...
第一步校验结构与结果内部约束；第二步必须由服务层调用，检查岗位绑定、分析范围与证据引用。引用仅指向标准输入中的字符串，不代表PDF原文溯源；
...
class AnalysisEvidence(_AgentOutputModel):
    ...
    quote: NonEmptyText = Field(description="指定字段中逐字存在的文本片段，不允许改写或虚构")
...
class DimensionAssessment(_AgentOutputModel):
    ...
    rationale: NonEmptyText = Field(description="基于JD要求和简历事实解释判断")

    evidence: list[AnalysisEvidence] = Field(description="引用当前JD要求及相关简历事实；无依据时可为空")

    missing_information: list[NonEmptyText] = Field(description="需要候选人或面试官补充的信息；无缺失时返回空列表")
    ...
    def validate_assessment(self) -> DimensionAssessment:
        if self.status in {"unknown", "not_applicable"}:
            ...
            if self.score is None:
                raise ValueError("已作出匹配判断的维度必须提供score")
            if {item.source for item in self.evidence} != {"resume", "jd"}:
                raise ValueError("匹配判断必须同时引用简历事实和当前JD要求")
...
class AnalysisFinding(_AgentOutputModel):
    ...
    def validate_evidence_sources(self) -> AnalysisFinding:
        if {item.source for item in self.evidence} != {"resume", "jd"}:
            raise ValueError("优势或差距必须同时引用简历事实和当前JD要求")
        return self


class InterviewQuestion(_AgentOutputModel):
    """面试问题，包含问题依据和面试官关注关注点。"""
    dimension: AssessmentDimension
    question: NonEmptyText = Field(description="围绕岗位要求、经历或待核实信息提出的问题")
    purpose: NonEmptyText = Field(description="该问题需要验证的能力或事实")
    evidence: list[AnalysisEvidence] = Field(min_length=1, description="问题依据，可引用岗位要求或简历相关经历")
    evaluation_points: list[NonEmptyText] = Field(min_length=1, description="面试官应关注的回答要点，不是候选人已具备能力的断言")
...
class JobAnalysis(_AgentOutputModel):
    ...
    jd_id: int
    target_ids: list[NonEmptyText] = Field(min_length=1, description="输入中selected_jd_id指向本JD的全部target_id")
    ...
    overall_score: float | None = Field(
        ...
        description=(
            ...
            "跨候选人比较须由服务层固定JD与评分rubric。"
        ),
    )
    overall_score_reason: NonEmptyText = Field(description="解释整体评分及不确定性；不评分时说明原因")
    dimensions: AssessmentDimensions
    strengths: list[AnalysisFinding]
    gaps: list[AnalysisFinding] = Field(description="有证据支持的不匹配项；简历未提及的信息不能直接写成能力不足")
    interview_questions: list[InterviewQuestion] = Field(min_length=1)

    @model_validator(mode="after")
    def validate_job_analysis(self) -> JobAnalysis:
        if len(self.target_ids) != len(set(self.target_ids)):
            raise ValueError("同一JD的target_ids不能重复")
        assessments = [
            getattr(self.dimensions, name)
            for name in AssessmentDimensions.model_fields
        ]
        if self.overall_score is not None and all(item.score is None for item in assessments):
...
class AgentAnalysisOutput(_AgentOutputModel):
    ...
    job_analyses: list[JobAnalysis] = Field(min_length=1, description="按JD去重，覆盖输入target_matches中的全部已确定岗位")
    ...
    def validate_uniqueness(self) -> AgentAnalysisOutput:
        ...
        if len(jd_ids) != len(set(jd_ids)):
            raise ValueError("job_analyses中的jd_id不能重复")
        target_ids = [target_id for job in self.job_analyses for target_id in job.target_ids]
        if len(target_ids) != len(set(target_ids)):
            raise ValueError("一个target_id只能出现在一份JD分析中")
        return self

    def validate_against_input(self, agent_input: AgentAnalysisInput) -> AgentAnalysisOutput:

        """服务层在保存/展示前调用；失败抛出ValueError，可进入有限次修复重试。
        以输入中target_matches的绑定为准，禁止增加待确认岗位、遗漏已确定岗位、
        改绑JD或把partial分析标记为complete。证据校验只验证引用存在性。
        """
        ...
        if actual != expected:
            raise ValueError("输出的target_id/JD绑定必须完整且精确地对应输入target_matches")
        ...
        for job in self.job_analyses:
            ...
            for item in evidence_items:
                ...
                if item.quote not in value:
                    raise ValueError(
                        f"JD {job.jd_id}: 证据quote不存在于 {item.source}{item.path}"
                    )
        return self
...
def _resolve_text_pointer(document: dict, pointer: str) -> str:
    ...
    for raw_token in pointer[1:].split("/"):
        ...
        if isinstance(value, dict) and token in value:
            ...
            index = int(token)
            if index >= len(value):
                raise ValueError(f"证据path的数组下标越界: {pointer}")
            value = value[index]
        else:
            raise ValueError(f"证据path不存在: {pointer}")
    if not isinstance(value, str) or not value.strip():
        raise ValueError(f"证据path必须指向非空字符串: {pointer}")
    return value
