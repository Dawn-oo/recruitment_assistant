...
默认连接 DeepSeek；使用官方支持的 json_object 模式，再执行本地结构和业务校验。
JSON 模式不能保证满足 Schema，更不能保证事实正确。
每次 analyze_job 最多发起一次 SDK 请求，重试与退避由服务层/state.py 统一管理。
不读取项目配置或密钥文件；由组合根传入 api_key、配置或已有 AsyncOpenAI 实例。
...
retry_pending 的等待、退避与再次 start_job 由调用方安排；不要额外套用 SDK 重试。
取消异常原样传播。进程中断后 running 状态的恢复策略也由调度层负责。
"""
...
from app.services.recruitment_agent.prompts import (
...
class AgentLLMConfig(BaseModel):
    ...
    timeout_seconds: float = Field(default=120.0, gt=0, description="单次 API 调用总等待上限")
...
class RecruitmentLLMClient:
    """可跨多个 JD 复用的异步客户端；自行创建的 SDK 实例需通过 aclose 关闭。"""

    def __init__(
        self, *, api_key: str | None = None, config: AgentLLMConfig | None = None,
        client: AsyncOpenAI | None = None,
    ) -> None:
        self.config = config or AgentLLMConfig()
        if client is not None and api_key is not None:
            raise ValueError("传入 client 时不要同时传入 api_key")
        if client is None and (not isinstance(api_key, str) or not api_key.strip()):
            raise ValueError("未传入 client 时必须显式提供非空 api_key")
        ...
        # 注入的 SDK 也按请求关闭内部重试，避免绕过状态机中的重试上限。
        # 注入时沿用该实例的 endpoint/凭据，config.base_url 不覆盖它。
        self._request_client = self._client.with_options(
            max_retries=0, timeout=self.config.timeout_seconds,
        )
        self._closed = False

    async def analyze_job(self, agent_input: AgentAnalysisInput, *, jd_id: int) -> JobAnalysis:
        """返回已验证的单 JD 报告；本方法不更改 state，不执行自动修复或重试。"""
        if self._closed:
        ...
        try:
            try:
                ...
                raise AgentLLMError(
                    "标准输入无效或 JD 未绑定", stage="input_prepare",
            ...
            try:
                ...
                raise AgentLLMError(
                    "报告未通过岗位绑定、证据引用或评分规则校验", stage="output_validate",
                    code="report_validation_failed", retryable=True,
                ) from None
        except AgentLLMError as exc:
            logger.warning(
    ...
    async def _request(self, system_prompt: str, user_prompt: str) -> ChatCompletion:
        try:
            # SDK 的 HTTP 超时限制单次 I/O 等待；wait_for 另外约束整个请求的等待时间。
            ...
            raise AgentLLMError(
                "模型请求超时", stage="llm_call", code="timeout", retryable=True,
            ) from None
        except APIConnectionError:
            raise AgentLLMError(
                "无法连接模型服务", stage="llm_call", code="connection_error", retryable=True,
            ) from None
        except APIStatusError as exc:
            status = exc.status_code
            raise AgentLLMError(
                f"模型服务返回 HTTP {status}", stage="llm_call", code=f"http_{status}",
            ...
            raise AgentLLMError(
                "模型服务响应格式异常", stage="llm_call", code="invalid_api_response", retryable=True,
            ) from None
        except APIError:
            raise AgentLLMError(
                "模型 SDK 调用失败", stage="llm_call", code="sdk_error", retryable=False,
            ) from None
    ...
    def _parse_report(response: ChatCompletion) -> JobAnalysis:
        ...
        if not isinstance(choices, list) or len(choices) != 1:
            raise AgentLLMError(
                "模型响应缺少唯一的分析结果", stage="output_parse",
                code="invalid_choices", retryable=True,
            )
        choice = choices[0]
        ...
        if getattr(message, "refusal", None) or finish_reason == "content_filter":
            raise AgentLLMError(
                "模型未提供本次分析", stage="llm_call", code="model_refusal", retryable=False,
            )
        if finish_reason == "length":
            raise AgentLLMError(
                "模型输出被截断，请调整 max_tokens 或输入长度后重新执行",
                stage="output_parse", code="output_truncated", retryable=False,
            )
        if finish_reason != "stop" or getattr(message, "tool_calls", None):
            raise AgentLLMError(
                "模型未正常完成 JSON 输出", stage="output_parse",
                code="unexpected_finish", retryable=True,
            )
        content = getattr(message, "content", None)
        if not isinstance(content, str) or not content.strip():
            raise AgentLLMError(
                "模型返回空内容", stage="output_parse", code="empty_output", retryable=True,
            )
        try:
            # 不提取围栏、不拼接截断 JSON、不将宽松修复后的内容当作有效报告。
