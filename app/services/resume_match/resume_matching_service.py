...
from app.services.resume_match.candidate_selector import (
...
from app.services.resume_match.vector_match.candidate_aggregator import (
...
class JobMatchingService:
    ...
    def match_resume(
        ...
        *,
        resume: ResumeModel,
        requested_job_titles: Sequence[str] | None = None,
        ...
        Args:
            ...
            requested_job_titles:
                已经由 JobIntentNormalizer 拆分、清理后的申请岗位列表。
                没有明确申请岗位时可以传 None 或空列表，此时只执行
                语义匹配分支。

            top_k_per_query:

                每个 ResumeQueryUnit 从 pgvector 召回的 chunk 数量；未传时
                使用服务初始化配置。
            semantic_top_n:
        ...
        normalized_titles = self._normalize_job_titles(
            requested_job_titles or ()
        )

        started_at = time.perf_counter()
        logger.info(
            "岗位匹配流程开始: requested_job_count=%d "
            "top_k_per_query=%d semantic_top_n=%d",
            len(normalized_titles),
            actual_top_k,
            actual_semantic_top_n,
        )

        exact_result, exact_warnings = self._run_exact_branch(
            requested_job_titles=normalized_titles,
        )
        ...
        logger.info(
            ...
            len(exact_result.matched_jd_ids) if exact_result else 0,
    ...
    def _run_exact_branch(
        self,
        *,
        requested_job_titles: Sequence[str],
    ) -> tuple[ExactJobMatchResult | None, list[str]]:
        """执行岗位名称精确匹配；没有岗位名称时正常跳过。"""
        if not requested_job_titles:
            logger.debug("没有明确申请岗位，跳过精确匹配分支")
            return None, []

        try:
            result = self._exact_matcher.match(
                requested_job_titles
            )
        except Exception:
            logger.exception(
                "岗位名称精确匹配失败: requested_job_count=%d",
                len(requested_job_titles),
            )
            return None, [
                "岗位名称精确匹配失败，本次仅使用语义匹配结果"
            ]

        logger.debug(
            "精确匹配分支完成: intent_count=%d matched_jd_count=%d "
            "unmatched_count=%d",
            len(result.intent_results),
            len(result.matched_jd_ids),
            len(result.unmatched_titles),
        )
        return result, []
    ...
    def _run_semantic_branch(
        ...
        *,
        ...
        try:
            ...
            retrieval_result = self._vector_retriever.retrieve_many(
                query_units,
